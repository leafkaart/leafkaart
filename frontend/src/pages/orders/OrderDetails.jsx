...
export default function OrderDetail() {
  ...
  const handleAssignDealer = async () => {
    if (!selectedDealer) {
      addToast("Please select a dealer", "error");
      return;
    }
    ...
    try {
      ...
      addToast("Dealer assigned successfully!", "success");
  ...
  const handleUnassignDealer = async () => {
    ...
    try {
      ...
      addToast("Dealer unassigned successfully!");
      refetch();
    } catch (error) {
  ...
  const handleUpdatePaymentStatus = async () => {
    try {
      ...
      addToast(
        `Payment marked as ${
          !order.paymentStatus ? "verified" : "unverified"
        }!`,
        "success"
      );
      refetch();
                  ...
                  <div className="p-4 bg-green-50 rounded-lg">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right text-sm text-gray-600">
                        <p>
                          Assigned by: {order.dealerAssign.assignedBy?.name}
                        </p>
                        <p className="text-xs">
                          {new Date(
                                ...
                                <div className="mt-3 pt-3 border-t">
                                  {hasMatches ? (
                                    ...
                                    <div className="text-center py-4 bg-gray-50 rounded-lg">
                                      <p className="text-sm text-gray-500">
                                        ⚠️ No matching products available for
                                        this order
                                      </p>
                                      <p className="text-xs text-gray-400 mt-1">
                                        This dealer doesn't have the ordered
                                        items in stock
                                      </p>
                                    </div>
                                  )}
