...
const EmployeeList = () => {
  ...
  const toggleAccountStatus = async (emp) => {
    setLoadingEmployeeId(emp._id);

    try {
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/auth/updateEmployee/${emp._id}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
        ...
        <div className="bg-white rounded-xl shadow-sm max-h-[calc(100vh-100px)] overflow-y-auto border border-gray-100">
          <table className="w-full text-left table-fixed">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                <th className="py-3 px-4 font-medium text-gray-600 text-sm w-[25%]">
                  Name
                </th>
                <th className="py-3 px-4 font-medium text-gray-600 text-sm w-[30%]">
                  Email
                </th>
                <th className="py-3 px-4 font-medium text-gray-600 text-sm w-[18%]">
                  Contact
                </th>

                <th className="py-3 px-4 font-medium text-gray-600 text-sm w-[10%]">
                  Status
                </th>
                <th className="py-3 px-4 font-medium text-gray-600 text-sm w-[120px] text-center">
                  Actions
                </th>
              </tr>
            </thead>
                  ...
                  >
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center justify-center gap-3">
                        {/* Toggle */}
                        <button
                          onClick={() => toggleAccountStatus(emp)}
                          disabled={loadingEmployeeId === emp._id}
                          className={`relative inline-flex shrink-0 items-center w-12 h-6 rounded-full transition-colors duration-300 focus:outline-none ${
                            emp.isActive ? "bg-green-500" : "bg-gray-300"
                          } ${
                            loadingEmployeeId === emp._id
                          ...
                          {loadingEmployeeId === emp._id ? (
                            ...
                            <span
                              className={`inline-block w-5 h-5 bg-white rounded-full shadow transform transition-transform duration-300 ${
                                emp.isActive ? "translate-x-6" : "translate-x-1"
                              }`}
                            />
                          )}
                        </button>
